...
if ($action==='suspend') $db->execute("UPDATE students SET is_suspended=1 WHERE id=?",[$id]);
elseif ($action==='unsuspend') $db->execute("UPDATE students SET is_suspended=0 WHERE id=?",[$id]);
